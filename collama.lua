...
local GEMINI_API_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=" .. GEMINI_API_KEY
...
local button = toolbar:CreateButton(
	pluginName,
	"Generate UI with LlaMa AI",
	"rbxassetid://6538721725"
)
...
local function generateCodeWithGemini(promptText)
	...
	if not responseWrapper or not responseWrapper.Success then
		statusLabel.Text = "API request failed (" .. (responseWrapper and responseWrapper.StatusCode or "N/A") .. "). Check Output."
	...
	if responseData.candidates and responseData.candidates[1] and
		responseData.candidates[1].content and responseData.candidates[1].content.parts and
		responseData.candidates[1].content.parts[1] and responseData.candidates[1].content.parts[1].text then
...
generateButton.MouseButton1Click:Connect(function()
	if not generateButton.Active then return end
